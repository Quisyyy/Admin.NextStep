...
async function ensureSupabaseReady(timeout = 3000) {
    ...
    while (Date.now() - start < timeout) {
        ...
        if (window.supabase && window.supabase.from && window.supabaseClientReady !== false) {
...
async function loadArchivedAlumni() {
    try {
        ...
        try {
            // Query with extensive null checking
            const queryResult = window.supabase
                .from('alumni_archive')
                .select('id, original_alumni_id, is_restored')
            ...
            if (!result) {
                console.error('❌ Awaited result is null');
                allArchivedRecords = [];
                renderArchiveTable([]);
                updateArchiveStats();
                return;
            ...
            console.log('Query result received:', typeof result, Object.keys(result || {}));
            ...
            // If we have records, get alumni details
            if (archiveRecords.length > 0) {
                const archiveIds = archiveRecords.map(r => r.original_alumni_id).filter(Boolean);
                
                if (archiveIds.length > 0) {
                    const alumniResult = await window.supabase
                        .from('alumni_profiles')
                        .select('id, full_name, email, student_number, degree')
                        .in('id', archiveIds);

                    const alumniData = alumniResult?.data || [];

                    allArchivedRecords = archiveRecords.map(archiveRecord => {
                        const alumniInfo = alumniData.find(a => a.id === archiveRecord.original_alumni_id) || {};
                        return {
                            id: archiveRecord.id,
                            original_alumni_id: archiveRecord.original_alumni_id,
                            is_restored: archiveRecord.is_restored,
                            full_name: alumniInfo.full_name || 'N/A',
                            email: alumniInfo.email || 'N/A',
                            student_number: alumniInfo.student_number || 'N/A',
                            degree: alumniInfo.degree || 'N/A',
...
function renderArchiveTable(records) {
    ...
    if (!records || records.length === 0) {
        ...
        tr.innerHTML = '<td colspan="8" class="empty" style="text-align: center; padding: 20px;">No archived records</td>';
        tbody.appendChild(tr);
        return;
    }

    records.forEach(record => {
        const archivedDate = record.archived_at ? new Date(record.archived_at) : new Date();
        const daysArchived = Math.floor((Date.now() - archivedDate.getTime()) / (1000 * 60 * 60 * 24));
...
function updateArchiveStats() {
    ...
    // Count restored
    const { data: restoredData } = window.supabase && window.supabase.from ? 
        null : { data: [] };
    ...
    if (header) {
        header.textContent = totalArchived === 0 
            ? '✅ No archived records' 
...
async function deleteRecord(archiveId, fullName) {
    if (!confirm(`⚠️ PERMANENTLY DELETE "${fullName}"? This cannot be undone!`)) return;
...
async function cleanupOldRecords() {
    try {
        ...
        if (!confirm(`Delete ${oldRecords.length} record(s) older than 30 days?`)) return;
...
function applyArchiveFilters() {
    ...
    const statusFilter = document.getElementById('filterArchiveStatus')?.value || '';
